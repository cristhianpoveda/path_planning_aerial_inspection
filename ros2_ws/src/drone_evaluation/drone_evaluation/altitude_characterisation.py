...
# def read_bag(bagpath, alt_topic, speed_topic, mocap_topic):
#     """Return dict of numpy arrays, timestamps in seconds (bag record time, zeroed).
#     Uses rosbag2_py so custom types (e.g. drone_interfaces/AltitudeAglStamped) resolve
#     from the sourced workspace. Requires: source install/setup.bash"""
#     import rosbag2_py
...
#     if not has_speed:
#         print(f"  [info] speed topic '{speed_topic}' not in this bag -> skipping velocity.")
...
#     ap.add_argument("--alt-topic", default="/drone_1/altitude_agl")
#     ap.add_argument("--speed-topic", default="/drone_1/speed_vector")
#     ap.add_argument("--mocap-topic", default="/optitrack/rigid_bodies/dji_mini4")
#     ap.add_argument("--vertical-axis", default="auto", choices=["auto", "x", "y", "z"])
...
#     d = read_bag(args.bag, args.alt_topic, args.speed_topic, args.mocap_topic)
...
#         axis, _ = pick_vertical_axis(d["moc_p"])
#     else:
...
#               f"({'sufficient' if yaw_range > np.radians(60) else 'INSUFFICIENT -> body~world, frame cannot be resolved'})")
